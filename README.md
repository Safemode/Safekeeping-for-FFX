...
- Sphere Grid Planner - a pan/zoom view of the full Standard (860 nodes) and Expert (805 nodes) Sphere Grids, switchable via the grid-type selector. Tap a node to see its content, edit it (Clear Sphere customization), or activate it. Node edits are shared like the real grid, while each of the 7 characters tracks their own activated path. The Standard grid opens on the character you are viewing - the node they most recently activated, or where the game starts them if they have not begun. Each grid remembers its own pan/zoom position after that, with a button to reset the view.
